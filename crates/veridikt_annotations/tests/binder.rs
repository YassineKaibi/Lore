use std::path::Path;
use veridikt_annotations::{BoundBlock, scan_source};
use veridikt_intent::Finding;
...
fn unbound_block_is_e0102() {
    let (bound, findings) = bind_py("# @veridikt\n# purpose: \"floating\"\nprint(1)\n");
...
fn scoping_block_without_name_is_e0108() {
    let (_, findings) = bind_py("# @veridikt\n# kind: module\nx = 1\n");
...
fn multi_declarator_const_without_name_is_e0104() {
    let (bound, findings) = bind_ts("// @veridikt\n// kind: state\nconst a = 1, b = 2;\n");
    assert!(bound.is_empty());
    assert_eq!(findings[0].code, "E0104");
}
