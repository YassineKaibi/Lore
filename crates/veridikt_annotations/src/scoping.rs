...
use veridikt_intent::{Finding, Kind, QName, Span};
    ...
    pub(crate) fn compile(globs: &'a [ModuleGlob]) -> Self {
        CompiledGlobs(
            globs
                .iter()
                .filter_map(|g| {
                    // `*` is confined to a single path segment; `**` is what
                    // crosses `/` (D-082). This is what makes init's
                    // `<root>/*` catch-all genuinely disjoint from a
                    // `<root>/<dir>/**` glob (no spurious E0103, §7.5).
                    GlobBuilder::new(&g.glob)
                        .literal_separator(true)
                        .build()
                        .ok()
                        .map(|c| (c.compile_matcher(), g))
                })
                .collect(),
        )
    }
...
pub(crate) fn scope_file(
        ...
        && let Some(other) = matches.iter().find(|g| g.module != first.module)
    {
        findings.push(Finding::new("E0103", file_span.clone(), format!(
            "{} is claimed by two modules: \"{}\" (glob \"{}\") and \"{}\" (glob \"{}\"); make the [modules] globs in veridikt.toml disjoint",
    ...
    if has_orphan_subjects {
        ...
        findings.push(Finding::new("W0208", file_span, format!(
            "{} belongs to no module, so its subjects get the \"_orphan\" qname prefix; map it under [modules] in veridikt.toml or add a top-of-file module block",
            file.path.display())));
    }
    (out, module)
}
