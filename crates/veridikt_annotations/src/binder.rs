...
use veridikt_intent::{Finding, Kind, PackSpec, Span};
        ...
        for block in blocks {
            ...
            if row >= lines.len() {
                findings.push(Finding::new("E0102", block_span,
                    "this @veridikt block is not followed by a bindable declaration (found end of file); move it directly above a function, class, type, or assignment".into()));
                continue;
            }
            ...
            let Some(first) = candidates.first().copied() else {
                findings.push(Finding::new("E0102", block_span,
                    "this @veridikt block is not followed by a bindable declaration (found nothing parseable); move it directly above a function, class, type, or assignment".into()));
                continue;
            };
            let Some(node) = candidates
                ...
                findings.push(Finding::new("E0102", block_span, format!(
                    "this @veridikt block is not followed by a bindable declaration (found {}); move it directly above a function, class, type, or assignment", first.kind())));
                continue;
            };
            ...
            if identifier.is_none() && block.name.is_none() {
                findings.push(Finding::new("E0104", block_span,
                    "this declaration has multiple targets; add an explicit \"name:\" field to the @veridikt block to choose the subject".into()));
                continue;
            }
        ...
        if block.name.is_none() {
            if scoping {
                ...
                findings.push(Finding::new("E0109", span,
                    "this language's pack is scan-tier (no grammar), so a non-scoping @veridikt block cannot find its subject; add an explicit \"name:\" field (§8.6.2)".into()));
            }
            continue;
        }
        bound.push(BoundBlock {
