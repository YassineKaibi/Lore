//! Boundary tests (G-4): `lore ask` command in -> exact stdout (human and
...
fn unparseable_query_is_exit_2_with_the_parse_message() {
    let out = lore(&["ask", "affects Payment.ledger"], &fixture("ask_project"));
...
fn unresolved_argument_is_exit_2_naming_the_nearest_qname() {
    let out = lore(&["ask", "affects(Payment.legder)"], &fixture("ask_project"));
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert_eq!(
        stderr,
...
fn ask_without_manifest_is_e0402_exit_2() {
    ...
    let out = lore(&["ask", "unknown"], tmp.path());
    assert_eq!(out.status.code(), Some(2));
...
fn unknown_query_prints_the_unknown_strings_indented() {
    let out = lore(&["ask", "unknown", "--no-color"], &fixture("ask_project"));
    assert_eq!(out.status.code(), Some(0));
    assert_eq!(
        String::from_utf8_lossy(&out.stdout),
        "unknown: 1 result\n\
...
fn show_works_for_a_node_without_declared_intent() {
    ...
    assert!(
        stdout.starts_with("Payment  Module  Declared  lore.toml:1\n"),
        "{stdout}"
    );
...
fn unknown_query_json_carries_the_unknown_strings() {
    let out = lore(&["ask", "unknown", "--json"], &fixture("ask_project"));
    let v: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(
        v["results"][0]["unknown"],
