...
//! and `lore stats` with the derivation counters (D-065).
...
fn call_graph_queries_answer_on_a_repo_with_zero_annotations() {
    let out = lore(
        &["ask", "triggers(Payment.charge)", "--no-color"],
        &fixture("derive_calls"),
    );
    assert_eq!(out.status.code(), Some(0));
    assert_eq!(
        String::from_utf8_lossy(&out.stdout),
        "triggers(Payment.charge): 2 results\n\
...
fn stats_reports_kind_origin_counts_and_the_drop_counters() {
    let out = lore(&["stats", "--no-color"], &fixture("derive_project"));
    assert_eq!(out.status.code(), Some(0));
...
fn stats_json_field_names_are_pinned() {
    let out = lore(&["stats", "--json"], &fixture("derive_project"));
    assert_eq!(out.status.code(), Some(0));
...
fn a_second_run_over_the_cache_answers_identically() {
    let q = &["ask", "triggers(Payment.charge)", "--no-color"];
    let first = lore(q, &fixture("derive_calls"));
    let second = lore(q, &fixture("derive_calls"));
    assert_eq!(first.stdout, second.stdout);
    assert!(fixture("derive_calls").join(".lore-cache/derive").is_dir());
}
