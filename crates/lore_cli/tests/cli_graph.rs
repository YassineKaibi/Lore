//! Boundary tests for `lore graph --dot` (D-038, §12). Unhappy path first
...
fn without_dot_flag_is_usage_error() {
    let out = lore(&["graph"], &fixture("ask_project"));
...
fn dot_output_is_a_wellformed_digraph() {
    let out = lore(&["graph", "--dot"], &fixture("ask_project"));
    assert_eq!(out.status.code(), Some(0));
    let dot = String::from_utf8_lossy(&out.stdout);
    assert!(dot.starts_with("digraph lore {"));
...
fn focus_with_depth_bounds_the_neighborhood() {
    let full = lore(&["graph", "--dot"], &fixture("ask_project"));
    let focused = lore(
        &[
            "graph",
            "--dot",
