...
//!
//! Golden files: set `LORE_BLESS=1` to (re)generate every `expected.json` from
...
use lore_annotations::{ActivePack, ScanConfig, ScanResult, SourceFile};
use lore_cli::manifest::{self, Manifest};
use lore_cli::packs::{self, LoadedPack};
use lore_derive::{DeriveConfig, DerivePack, DeriveResult, SourceUnit, StateSymbol};
use lore_intent::{ImportStrategy, Tier};
...
fn builtin_packs_pass_their_conformance_suites() {
    let bless = std::env::var_os("LORE_BLESS").is_some();
            ...
            for case in case_dirs(&class_dir) {
                ...
                let want: Value = serde_json::from_str(
                    &std::fs::read_to_string(&expected_path).unwrap_or_else(|_| {
                        panic!(
                            "missing {} — run with LORE_BLESS=1 to generate",
...
fn run_case(class: &str, case: &Path, pack: &LoadedPack) -> Value {
    ...
    let scan = lore_annotations::scan(
        &ScanConfig {
...
/// `lore_derive::derive`. State symbols come from the scanned `State` blocks;
...
fn run_derive(
    ...
    lore_derive::derive(&config, &[derive_pack], &units, &states)
}
...
fn scan_json(r: &ScanResult) -> Value {
    ...
    let findings: Vec<Value> = r
        ...
        .map(|f| {
            serde_json::json!({
                "code": f.code,
                "severity": match f.severity { lore_intent::Severity::Error => "error", lore_intent::Severity::Warning => "warning" },
...
fn edge_kind(k: lore_derive::DerivedEdgeKind) -> &'static str {
    match k {
        lore_derive::DerivedEdgeKind::Calls => "Calls",
        lore_derive::DerivedEdgeKind::Affects => "Affects",
        lore_derive::DerivedEdgeKind::Reads => "Reads",
    }
}

fn confidence(c: lore_derive::DerivedConfidence) -> &'static str {
    match c {
        lore_derive::DerivedConfidence::Exact => "Exact",
        lore_derive::DerivedConfidence::Resolved => "Resolved",
        lore_derive::DerivedConfidence::Heuristic => "Heuristic",
    }
}

// ---- fixture I/O ----

/// Each case carries a `lore.toml` giving `[modules]` and `[project] roots` —
/// the case is a self-describing mini-project, so the real §7.5/§8.2 rules run.
fn load_case_manifest(case: &Path) -> Manifest {
    let path = case.join("lore.toml");
    let text = std::fs::read_to_string(&path)
        .unwrap_or_else(|_| panic!("fixture case {} needs a lore.toml", case.display()));
    manifest::parse(&path, &text).expect("fixture lore.toml is valid")
}
