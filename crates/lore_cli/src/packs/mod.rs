...
//! E041x). `lore_cli` is the only crate that touches grammar crates: it
...
/// so `lore_intent` stays tree-sitter-free, D-070d). `grammar` is `None` at
...
pub fn from_dir(dir: &Path) -> std::io::Result<PackSource> {
    ...
    let name = dir
        ...
        .unwrap_or_default();
    let manifest = std::fs::read_to_string(dir.join("lore-lang.toml"))?;
    ...
    Ok(PackSource {
        name,
        manifest_path: dir.join("lore-lang.toml"),
        manifest,
                ...
                && other != pack.spec.name
            {
                collision.get_or_insert_with(|| {
                    Finding::new(
                        "E0410",
                        Span {
                            file: PathBuf::from("lore-lang.toml"),
                            line: 1,
                            col: 1,
                            end_line: 1,
                            end_col: 1,
...
pub fn load(src: &PackSource) -> Result<LoadedPack, Finding> {
    ...
    let root: Value = src.manifest.parse().map_err(|e: toml::de::Error| {
        e0410(format!("lore-lang.toml is not valid TOML: {}", e.message()))
    })?;
    let root = root
        .as_table()
        .ok_or_else(|| e0410("expected a table at the top level of lore-lang.toml".into()))?;

    // [pack] table and the format gate, before anything else (§8.6.1).
    let pack = root
        .get("pack")
        .and_then(Value::as_table)
        .ok_or_else(|| e0410("missing required [pack] table in lore-lang.toml".into()))?;
            ...
            "custom" => {
                ...
                if !is_custom_strategy(&name) {
                    return Err(e0414(format!(
                        "unknown custom import strategy \"{name}\"; no such strategy is registered in lore_derive (D-071b)"
                    )));
                }
...
/// The set of custom import-strategy names registered in `lore_derive`
/// (D-071b). Mirrors `lore_derive::custom_strategies()`; validated here so a
/// typo fails at load with `E0414`.
fn is_custom_strategy(name: &str) -> bool {
    lore_derive::custom_strategy_names().contains(&name)
}
