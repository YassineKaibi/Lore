...
pub struct DeriveConfig {
    ...
    /// `.veridikt-cache/` location; None disables the cache (D-064).
...
/// `veridikt_intent` stays tree-sitter-free (D-070d). `veridikt_cli` builds these from
...
/// veridikt_graph (§13); the CLI maps these onto graph edges.
