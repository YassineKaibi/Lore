...
use lore_intent::{Finding, Intent, IntentNode, Kind, Span};
...
fn occurrence(intent: &Intent, clause: Clause) -> Option<&Span> {
    fn first<T>(v: &[lore_intent::Spanned<T>]) -> Option<&Span> {
    ...
    match (req(clause, node.kind), present) {
        (Req::Required, None) => findings.push(OwnedFinding::new(
            Finding::new(
                ...
                node.loc.clone(),
                format!(
                    "{kind} \"{qname}\" is missing required \"{name}\"; add a {name} clause to its @lore block"
                ),
            ),
            qname,
        )),
