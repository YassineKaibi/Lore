...
//! promotion. Consumes data from veridikt_annotations/veridikt_derive — never the
...
use veridikt_intent::{Finding, IntentNode, Kind, QName, Span, Spanned};
...
/// The derived layer as data (§13): veridikt_derive's output mapped onto graph
...
/// `veridikt graph --focus <qname>` mirroring `veridikt ask`'s unknown-argument
...
impl Ctx {
    ...
    pub(crate) fn strict_module(&self, q: &QName) -> bool {
        use veridikt_intent::Enforcement;
