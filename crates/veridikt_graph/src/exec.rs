...
pub fn ask(graph: &Graph, query: &Query, options: &Options) -> Result<Answer, String> {
    ...
    let (mut hits, visited) = match &query.expr {
        ...
        // Reverse over Triggers∪Calls: the functions that call F.
        QueryExpr::Callers { arg, star } => call_edge(graph, arg, Direction::Reverse, *star)?,
        // Forward over Triggers∪Calls: the calls F makes -- the set a
        // `triggers:` clause declares, so this is how you ground one (D-083).
        QueryExpr::Callees { arg, star } => call_edge(graph, arg, Direction::Forward, *star)?,
...
/// `callers(F)` (reverse) / `callees(F)` (forward): one hop over the call
/// edge set `Triggers`∪`Calls`, transitive with `*` (§10.6, D-083).
fn call_edge(
    graph: &Graph,
    arg: &QName,
    direction: Direction,
...
/// failure — shared with `veridikt history` (D-059a), which mirrors ask's
