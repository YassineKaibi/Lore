...
use veridikt_graph::exec::{Answer, Hit, Options, ask};
use veridikt_graph::query::parse;
use veridikt_graph::{DerivedLayer, EdgeKind, Graph};
use veridikt_intent::{Intent, IntentNode, Kind, Origin, QName, Ref, Route, Span, Spanned};
...
fn build(
    ...
    veridikt_graph::build(
        ...
        codeowners,
        DerivedLayer::empty(),
        veridikt_graph::ReconcileInput::empty(),
    )
}
...
fn fixture() -> Graph {
    ...
    charge.route = Some(spanned(
        Route {
            method: Some(veridikt_intent::HttpMethod::Post),
            path: "/charge".into(),
        },
        18,
    ));
...
#[test]
fn callers_is_reverse_calls_only_and_never_the_event_hop() {
    let g = fixture();
    assert_eq!(
        qnames(&run(&g, "callers(PaymentService.charge)")),
        ["Onboarding.collect"]
    );
    // transitive over Triggers/Calls only: charge causes audit via the event
    // hop, but `callers*` must not see it (§6.4)
    assert_eq!(
        qnames(&run(&g, "callers*(Payment.audit)")),
        ["Onboarding.verify", "Payment.book"]
    );
}

#[test]
fn callees_is_the_forward_dual_of_callers() {
    // `callees(F)` returns the calls F makes -- the direction a `triggers:`
    // clause declares (D-083). It is the exact inverse of `callers`: since
    // Onboarding.collect calls PaymentService.charge, charge is among
    // collect's callees.
    let g = fixture();
    assert!(
        qnames(&run(&g, "callees(Onboarding.collect)"))
            .contains(&"PaymentService.charge".to_string())
    );
}
