...
use veridikt_annotations::ScanResult;
use veridikt_graph::exec::{Card, Hit};
use veridikt_graph::{ClaimStatus, Edge, Graph};
use veridikt_intent::{Finding, Intent, QName, Severity};
...
pub fn to_json(r: &ScanResult) -> serde_json::Value {
    ...
    serde_json::json!({
        "veridikt_version": env!("CARGO_PKG_VERSION"),
        "blocks": blocks,
...
pub fn lint_to_json(graph: &veridikt_graph::Graph, findings: &[Finding]) -> serde_json::Value {
...
pub fn render_ask(
    ...
    if !quiet && !unresolved.is_empty() {
        ...
        out.push_str(&format!(
            "note: {} unresolved ref{} in the graph (run veridikt lint): {}\n",
            unresolved.len(),
...
fn origin_name(node: &veridikt_intent::IntentNode) -> &'static str {
    match node.origin {
        veridikt_intent::Origin::Declared => "Declared",
        veridikt_intent::Origin::Derived => "Derived",
        veridikt_intent::Origin::Both => "Both",
    }
}
...
/// by veridikt_graph's tests: `unresolved ref "<ref>" in ...`.
...
fn intent_lines(intent: &Intent) -> Vec<String> {
    ...
    if let Some(e) = &intent.enforcement {
        let level = match e.value {
            veridikt_intent::Enforcement::Strict => "strict",
            veridikt_intent::Enforcement::Warn => "warn",
        };
...
fn intent_to_json(intent: &Intent) -> serde_json::Value {
    let prose = |v: &[veridikt_intent::Spanned<String>]| -> Vec<String> {
        v.iter().map(|s| s.value.clone()).collect()
    };
    let refs = |v: &[veridikt_intent::Spanned<veridikt_intent::Ref>]| -> Vec<String> {
        v.iter()
            .map(|r| QName(r.value.segments.clone()).to_string())
            .collect()
    };
...
fn method_name(m: veridikt_intent::HttpMethod) -> &'static str {
    match m {
        veridikt_intent::HttpMethod::Get => "GET",
        veridikt_intent::HttpMethod::Post => "POST",
        veridikt_intent::HttpMethod::Put => "PUT",
        veridikt_intent::HttpMethod::Delete => "DELETE",
        veridikt_intent::HttpMethod::Patch => "PATCH",
    }
}
