//! `veridikt init` (§12): write a starter veridikt.toml — detect languages, propose
...
pub fn run() -> i32 {
    ...
    let manifest_path = cwd.join("veridikt.toml");
    if manifest_path.exists() {
        eprintln!("veridikt.toml already exists here; veridikt init never overwrites it");
        return 2;
    }
    ...
    std::fs::write(&manifest_path, text)
        .expect("veridikt init writes into a directory it just read");

    println!("init: wrote veridikt.toml");
    println!(
        "  languages: {}",
...
/// (W0208, §7.5(3); D-081). The module matcher confines `*` to one path
/// segment (`**` crosses `/`, D-082), so `<root>/*` matches only direct file
/// children and never overlaps a `<root>/<dir>/**` glob (no E0103).
