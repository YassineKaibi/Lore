//! `veridikt graph --dot [--focus <qname> --depth N]` (§12, D-038): export the
...
pub fn run(
    ...
    if !dot {
        eprintln!("veridikt graph supports only --dot output in v1; pass --dot");
        return 2;
    }
    ...
    let included: BTreeSet<QName> = match focus {
        ...
        Some(f) => {
            ...
            if !graph.nodes.contains_key(&fq) {
                match veridikt_graph::nearest_qname(f, &graph) {
                    Some(n) => eprintln!(
...
fn to_dot(graph: &Graph, included: &BTreeSet<QName>) -> String {
    let mut out = String::from("digraph veridikt {\n  rankdir=LR;\n  node [shape=box];\n");
