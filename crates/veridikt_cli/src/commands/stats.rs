//! `veridikt stats` (§12, D-065): coverage counts — nodes by kind and origin,
//! declared-intent coverage per kind, edges by layer, the claims-by-status
//! breakdown (D-069), and the derivation drop counters.

use std::path::Path;

use veridikt_graph::{ClaimStatus, Layer};
use veridikt_intent::{Intent, Kind, Origin};
...
// because: "The drop counters are the honesty surface for the derived layer: every guess veridikt refused to make is visible here, not silently absent (G-7)"
