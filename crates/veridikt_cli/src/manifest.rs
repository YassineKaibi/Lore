//! `veridikt.toml` discovery, parsing, and validation (spec §11, D-043).
...
use toml::Value;
use veridikt_intent::{Finding, Span};
...
pub struct Manifest {
    ...
    pub modules: Vec<veridikt_annotations::ModuleGlob>, // TOML order preserved
    pub policy: Policy,
...
pub const W_CODES: [&str; 13] = [
    "W0110", "W0205", "W0206", "W0207", "W0208", "W0209", "W0210", "W0211", "W0212", "W0213",
    "W0301", "W0302", "W0303",
];
...
/// Walk up from `start` looking for veridikt.toml; None => caller reports E0402.
pub fn discover(start: &Path) -> Option<PathBuf> {
    start
        .ancestors()
        .map(|dir| dir.join("veridikt.toml"))
...
pub fn parse(path: &Path, text: &str) -> Result<Manifest, Finding> {
    ...
    let unknown_key = |key: &str| {
        Finding::new(
            "E0401",
            span(),
            format!("unknown key \"{key}\" in veridikt.toml; {VALID_KEYS}"),
        )
    };
    let invalid = |key: &str, detail: String| {
        Finding::new(
            "E0403",
            span(),
            format!("invalid value for \"{key}\" in veridikt.toml: {detail}"),
        )
    };
    ...
    let root: Value = text.parse().map_err(|e: toml::de::Error| {
        Finding::new(
            "E0403",
            span(),
            format!("veridikt.toml is not valid TOML: {}", e.message()),
        )
    })?;
        ...
        match key.as_str() {
            ...
            "modules" => {
                ...
                for (glob, v) in table {
                    ...
                    m.modules.push(veridikt_annotations::ModuleGlob {
                        glob: glob.clone(),
                ...
                for (code, v) in table {
                    ...
                    if !W_CODES.contains(&code.as_str()) {
                        return Err(Finding::new(
                            ...
                            span(),
                            format!(
                                "unknown key \"{code}\" in veridikt.toml [lint]; only §18 W-codes can be overridden ({})",
                                W_CODES.join(", ")
                            ),
                        ));
                    }
