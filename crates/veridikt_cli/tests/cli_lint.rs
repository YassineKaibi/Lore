...
fn lint_json_emits_exact_findings_and_exit_1() {
    let out = veridikt(&["lint", "--no-stale", "--json"], &fixture("lint_project"));
...
fn lint_without_manifest_is_e0402_exit_2() {
    ...
    let out = veridikt(&["lint"], tmp.path());
...
fn lint_off_overrides_suppress_findings_and_the_exit_surface() {
    ...
    let out = veridikt(&["lint", "--no-stale", "--no-color"], &fixture("lint_off"));
    assert_eq!(out.status.code(), Some(0));
    assert_eq!(
...
fn ci_sample_project_fails_on_its_seeded_e0201() {
    ...
    let out = veridikt(&["lint", "--no-stale", "--no-color"], &sample);
    assert_eq!(out.status.code(), Some(1));
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
