//! `veridikt history` against a fixture repo with scripted commits (T5 exit
...
fn history_for_an_unknown_qname_is_exit_2_with_the_nearest() {
    ...
    let out = veridikt(&["history", "Payment.charg"], repo.path());
...
fn history_outside_a_git_work_tree_is_exit_2() {
    ...
    .unwrap();
    let out = veridikt(&["history", "Payment.charge"], tmp.path());
    assert_eq!(out.status.code(), Some(2));
    assert!(
...
fn history_renders_the_scripted_commits_newest_first() {
    let repo = scripted_repo();
    let out = veridikt(&["history", "Payment.charge"], repo.path());
...
fn history_json_carries_full_hashes_and_messages() {
    let repo = scripted_repo();
    let out = veridikt(&["history", "Payment.charge", "--json"], repo.path());
    assert_eq!(out.status.code(), Some(0));
...
fn history_quiet_drops_the_header() {
    let repo = scripted_repo();
    let out = veridikt(&["history", "Payment.charge", "--quiet"], repo.path());
    assert_eq!(out.status.code(), Some(0));
    assert!(
