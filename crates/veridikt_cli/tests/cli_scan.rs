...
fn scan_json_emits_exact_block_set() {
    let out = veridikt(&["scan", "--json"], &fixture());
    let v: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(v["veridikt_version"], "0.2.0");
...
fn missing_manifest_is_e0402_exit_2() {
    ...
    let out = veridikt(&["scan"], tmp.path());
...
fn human_output_is_deterministic() {
    let a = veridikt(&["scan", "--no-color"], &fixture());
    let b = veridikt(&["scan", "--no-color"], &fixture());
