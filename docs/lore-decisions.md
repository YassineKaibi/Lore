...
**D-073 -- `derive.scm` touch vocabulary: write markers in data, the bare-occurrence scan in the engine (hybrid).**
Decision: §8.6.3's touch vocabulary, as first written (`@touch.assign_lhs`, `@touch.aug_assign_lhs`, `@touch.receiver`, `@touch.call_function`), could not reproduce §8.3 / D-062d as built: it had no capture for **bare read occurrences** (the pre-T8 engine scanned every identifier and validity-filtered in Rust -- `valid_bare_site`: drop declaration-name, keyword-argument, and member-attribute positions) nor for the **whole-import `alias.attr` access** (`svc.ledger.append(1)`, `n = svc.ledger`). The chosen split (the alternative -- pushing read-position exclusions into the query as tree-sitter negatives -- was rejected: tree-sitter has no clean "identifier not in a name field" predicate, so it would make every pack's `derive.scm` fragile, the opposite of D-070's intent):
(a) **Occurrence shapes are query data.** `derive.scm` captures occurrence sites by shape: `@touch.symbol` -- a bare identifier occurrence; `@touch.access` with `@touch.access.obj` + `@touch.access.attr` -- a whole-import `alias.attr` member access (D-062d). These give the engine the per-language member-access structure (object identifier + member identifier) without it knowing any node kinds.
(b) **Write markers are query data.** `@touch.assign_lhs` and `@touch.aug_assign_lhs` mark the assignment / augmented-assignment target (a bare identifier or an `@touch.access`); `@touch.receiver` + `@touch.call_function` mark a method call's receiver expression (identifier or access node) and its method name. An occurrence node that coincides with `@touch.assign_lhs`/`@touch.aug_assign_lhs`, or with a `@touch.receiver` whose `@touch.call_function` is in `[derive.mutators] methods`, is a **write**; every other matched occurrence is a **read** (§8.3 unchanged; confidence `Heuristic` always).
(c) **Validity and classification stay engine behavior** (D-070g, the hybrid choice). The bare-occurrence validity filter -- a `@touch.symbol` node is skipped when it is itself a declaration's host identifier (it coincides with a `bind.scm` `@subject.name` capture at that node) or is the member side of an `@touch.access` -- is generic engine logic, not asked of the query author. Keyword-argument exclusion likewise: an over-counted read is noise, never a wrong edge, so the engine errs toward dropping ambiguous bare occurrences (G-7 favours the conservative miss). The mutator-list application and the write/read decision are engine behavior identical for every pack; a pack supplies only the captures above and `[derive.mutators]`.
Rationale: the genuinely language-specific facts here are *where the occurrence and write shapes are* in each grammar -- exactly what a capture expresses well. The genuinely language-agnostic facts -- "this bare token is a real reference, not a binder name" and "a write coincides with a marked write site" -- are awkward to encode as queries and natural as engine code, so they stay there (D-070g already reserves binder mechanics to the engine; this extends the same line to the touch scan). One adapter, the four pre-T8 touch behaviors preserved, zero per-language Rust.
Affects: §8.6.3, lore_derive (T8), roadmap T8.

**D-074 -- Value-bound function boundaries (lambda/arrow) become pack data.**
Decision: D-062a's attribution rule -- a call or touch attributes to its *nearest enclosing derived Function*, and a call inside a value-bound function (Python `lambda`, TS arrow / function-expression) has no honest attribution and drops -- depends on the engine knowing which node kinds are value-bound functions, so the ancestry walk stops at one instead of climbing past it to an outer declaration. Pre-T8 this was a hardcoded per-language list (`LAMBDAS`). Under packs (D-070) it is pack data: `PackSpec.value_functions`, from `[derive] value_functions` (Python `["lambda"]`; TS `["arrow_function", "function_expression", "generator_function"]`). The walk and the drop are unchanged engine behavior (D-062a/D-070g); only the node-kind list relocates from Rust into the manifest, exactly like `wrappers`/`sibling_skips`/`mutators`. The field joins §13 `PackSpec` in the migration PR (G-3, the same commit as its first consumer); a missing/empty list simply means the language has no value-bound function form to stop at.
Rationale: this is the last per-language datum the pre-T8 `lore_derive` carried in Rust that the generic extractor needs; leaving it hardcoded would reintroduce exactly the per-language module D-070 bans. It is a node-kind list, the same shape the binder already takes as data.
Affects: §8.6.1, §13, lore_derive (T8), roadmap T8.

**D-075 -- A language pack's `fixtures/` suite is excluded from the project source walk.**
Decision: the CLI's source walk (the directory traversal that feeds `lore scan`/`lint`) skips the `fixtures/` directory of any language pack -- a directory named `fixtures` whose parent contains a `lore-lang.toml` (§8.6). Conformance fixtures are deliberately-malformed inputs (annotations that MUST fail to bind, calls that MUST drop -- §8.6.4); they are test data owned by the pack, exercised only by the conformance harness (D-070e), never project source under intent management. The same skip applies to the manifest-file collection walk (D-071's `manifest_prefix`), so a fixture's `go.mod` is never mistaken for the project's. This narrows the existing build-artifact skip set (`.git`, `target`, `node_modules`, `.lore-cache`, dot-dirs) by one *pack-scoped* case rather than skipping every directory named `fixtures` -- a user's own `fixtures/` source dir, not sitting beside a `lore-lang.toml`, is still scanned.
Rationale: the Lore repo dogfoods `lore lint` in CI and ships builtin packs whose fixtures include intentional `E0102`/orphan cases; without this, the repo's own conformance inputs would fail its own lint. The pack-scoped predicate (`lore-lang.toml` sibling) keeps the rule precise -- it is a property of *being a pack's fixture suite*, not of the directory name.
Affects: §8.6.4, lore_cli source walk (T8), roadmap T8.

**D-076 -- Implicit whole-import alias from the path tail (Go/Java).**
Decision: Go (`import "example.com/app/helpers"`) and Java (`import app.Helper;`) bind the *last path segment* (`helpers`, `Helper`) as the name used at call sites (`helpers.Hello()`, `Helper.run()`), not the full path. The extractor's whole-import handling defaulted a Whole import's implicit alias to the full source string -- correct for Python `import a.b` (which binds the *first* segment `a`; the full alias is harmless because no single identifier can address it, the pre-T8 behavior), wrong for Go/Java. Add an optional `[derive] whole_alias = "full" | "last_segment"` key (default `"full"`) to the §8.6.1 manifest table and `PackSpec`. When `"last_segment"`, the extractor sets a Whole import's implicit alias to the last segment of the import source split on the pack's import separator (`/` for Go's path strategies, `.` for Java's `root_relative`); an explicit `@import.namespace`/`@import.alias` capture still wins. Python and TypeScript keep the default, so their behavior is unchanged (G-7: no silent drift). The separator is the one the pack's own strategies already declare, so no new separator key is introduced.
Rationale: this is the minimal pack-data extension that lets the *generic* extractor produce the right binding name for the two languages whose package/type imports name by path tail; encoding it as Rust per-language logic would reintroduce exactly the per-language modules D-070 bans. It is the import-side analogue of D-074's value-function list.
Affects: §8.6.1, §8.6.3, §13 (`PackSpec`), lore_derive extractor (T8), roadmap T8.

**D-077 -- `package_dir` resolves bare same-package references.**
Decision: D-071 defines `package_dir` as serving "bare same-package references" -- a symbol defined in a sibling file of the same directory (package) and used with no import, the normal cross-file shape in Go and Java. The resolver previously tried import strategies only on *import specifiers*; a bare callee was looked up same-file only (§8.2 Exact), so cross-file same-package references dropped and `package_dir` resolved nothing. Extend §8.2 resolution: when the active pack configures a `package_dir` strategy, a Bare callee unresolved in its own file *and* not bound by a named import is retried across the other in-scope files **in the same directory** (the package); an unambiguous declaration of that name in a sibling file resolves with confidence `Resolved` (it crossed a file boundary -- not `Exact`, §8.4). Same-file resolution still wins first and stays `Exact`. The fallback is engine behavior gated on the strategy's presence in the pack, so packs without `package_dir` (Python, TypeScript, Rust) are entirely unaffected; ambiguous sibling names drop and are counted (G-7). This realizes the strategy D-071 already named for Go and Java rather than leaving it inert.
Rationale: without it, `package_dir` is dead configuration and Go/Java cross-file same-package calls -- the common case -- all drop, badly understating the graph; the directory-scoped, unambiguous-only rule keeps it honest (a guess is never presented as a fact). It mirrors the existing import-resolution drop/count discipline.
Affects: §8.2, §8.6.1, §8.6.2, lore_derive resolver (T8), roadmap T8.

**D-078 -- `rust_use_paths` resolves through the `mod`-declaration tree.**
Decision: implement the first custom import strategy (D-071b/c). `use crate::`/`self::`/`super::` paths resolve through Rust's `mod`-declaration tree, which is not a function of the directory tree (a `mod x;` can be absent, inline, or `#[path]`-redirected), so no path-shaped builtin can express it -- the escape hatch's reason to exist. (a) Extend the §8.6.3 derive capture vocabulary with module-declaration captures: `@module.decl` (the `mod` item), `@module.name` (its identifier), and `@module.inline` (the body node, present iff the module is inline). The generic extractor emits a per-file `ModFact { name, inline }` list (cacheable, part of `FileFacts`); this is the only Rust-specific *data*, captured generically. (b) `resolve.rs` builds the crate module tree from the `ModFact`s and `[project] roots`: a `lib.rs`/`main.rs` directly under a roots dir is a crate root with module path `["crate"]`; an external `mod x;` in a file at module path P maps child `P::x` to the sibling file `x.rs` or `x/mod.rs` under that file's module directory (the file's parent for a crate root or `mod.rs`, else parent`/`stem); an inline `mod x { }` maps `P::x` to the same file. (c) The tree is passed into the strategy via two `ProjectData` maps (`module_paths`: file→path, `modules_by_path`: path→file); for non-Rust packs they are empty. (d) `rust_use_paths(specifier, importer, data)` splits the specifier on `::`, resolves a leading `crate`/`self`/`super` against the importer's module path (`self` = importer's module, `super` = its parent), looks the resulting module path up in `modules_by_path`, and returns that file; anything else (a bare or external-crate path) drops and is counted (G-7). Rust is promoted from tier `bind` to `derive`; `rust_use_paths` was already reserved in `custom_strategy_names()`.
Rationale: this is the decision D-071c committed to, made concrete. Keeping the mod data as generic capture vocabulary + engine-built tree (only the *resolution* logic is in the named impl) honors D-070's "per-language adapters become data" while using the escape hatch exactly where a builtin cannot reach. Directory-convention resolution was explicitly rejected by D-071c.
Affects: §8.2, §8.6.3, §13 (`FileFacts`/`ProjectData` are internal, no §13 surface change), lore_derive (T8), roadmap T8; first consumer of D-071b's custom registry.
