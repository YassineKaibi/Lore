...
### T1 -- Scan and Bind
**Scope:** Workspace + `veridikt_intent` skeleton (types only), `veridikt_annotations` (scanner + tree-sitter binder for Python and TypeScript), `veridikt_cli` with `veridikt init` and `veridikt scan`. Manifest parsing (`veridikt.toml`: `[project]`, `[modules]`), module scoping rules (spec §7.5).
**Out:** clause parsing (blocks are captured as raw text), derivation, queries.
**Exit criteria:**
- `veridikt scan --json` on the fixture trees emits the exact expected block set: file, line span, subject identifier, qname, kind.
...
- `veridikt init` on a bare Python repo produces a valid manifest.
**Spec:** §7, §11, §12 (init/scan).

### T2 -- Intent Parse
**Scope:** `veridikt_intent` clause parser: full clause grammar, spans, multi-line strings, `Intent` struct exactly per §13. Unknown clause `E0202` with suggestion; duplicate singular clause `E0206`.
**Exit criteria:**
...
**Scope:** `veridikt_graph`: node table, both adjacency maps, resolution (§6.3), applicability matrix, `depends_on` surface checks, hygiene checks (`W0210`-`W0212`), duplicate qnames. `veridikt lint` (structural subset) with exit codes and `--json`. `enforcement: strict` promotion. **Dogfooding starts: annotate the Veridikt repo, lint in Veridikt's CI (G-12; requires enabling the Rust row of §7.4 -- scanner+binder only).**
**Exit criteria:**
...
- Veridikt's own CI runs `veridikt lint` on the Veridikt repo, green.
...
**Scope:** `veridikt lint` hardening for CI adoption: severity overrides (`[lint]`), `[policy]` promotion for `unknown`, stable output ordering, `--quiet`. `veridikt history <qname>` (git log -L). CODEOWNERS cross-check `W0207`.
**Exit criteria:**
- A GitHub Actions example workflow in the repo runs lint on a sample project and fails on a seeded `E0201`.
- `veridikt history` output verified against a fixture repo with scripted commits.
**Spec:** §9.3, §11 policy/lint tables, §12.

### T6 -- The Derived Layer (Python, TypeScript)
**Scope:** `veridikt_derive` per §8: derived nodes, `Calls` edges with Exact/Resolved resolution and drop-rule, state-touch `Affects`/`Reads` with mutator tables, confidence labeling end to end (queries now traverse `Calls`; outputs print confidence). Parse cache (G-9). Open-world resolution (D-017) switched on.
**Exit criteria:**
...
- `veridikt ask` call-graph queries return correct results on a fixture repo with **zero annotations**; `affects(X)` is exercised with only X's own state annotation (state touches presuppose declared State nodes, D-060).
- `veridikt stats` reports `unresolved_calls` and per-kind node counts.
...
**Scope:** Claim statuses per §9.1 (four-status algorithm, symbol-occurrence test), `W0302/E0302`, `W0303` behind policy, staleness `W0301` via git blame (§9.2), `veridikt stats` claim-status breakdown. All statuses visible in `ask`/`show` output.
**Exit criteria:**
...
**Scope:** Language-pack loader and generic adapter (§8.6, D-070): pack manifest validation (band `E041x`), tier semantics (scan/bind/derive), builtin packs embedded at build time. Built-in import-strategy library (`relative`, `root_relative`, `package_dir`, `manifest_prefix`) with the named-custom escape hatch (D-071). Conformance harness running every builtin pack's fixture suite through the real scan→bind→derive pipeline in CI. Python and TypeScript migrate onto packs (their existing fixtures become their conformance suites). **Go and Java land as the first pure packs** (zero per-language Rust); **Rust lands as a pack with one custom import strategy** (`rust_use_paths`, D-071c) -- all three completing their §7.4 + §8.5 rows (scanner, binder, derivation). `veridikt graph --dot` with `--focus/--depth`.
...
- Rust: pack loads with `rust_use_paths`; Veridikt's own repo (Rust) now reconciles, not just lints structurally.
...
### T9 -- MCP Server
**Scope:** `veridikt mcp` (stdio): tools `veridikt_ask`, `veridikt_show`, `veridikt_lint`, `veridikt_history`, returning the §10.4 JSON. Read-only by construction.
...
1. Five canonical onboarding questions per repo ("what writes to <state>?", "what is <node> for and why?", "what reaches <state> from a public entry point?", "what is unresolved in <service>?", "who owns <area>?") answered correctly by `veridikt ask`/`show`, verified manually against the code.
...
### L1 -- Lexer + Parser
Full grammar of spec §15 including NEWLINE rules (D-034), `with`, `?`, `emit`, error unions. Exit: every `.veridikt` example in the spec parses to the exact expected AST; every E-band parse error fixture produces its code; malformed-input fuzzing (1h, no panics). Unhappy path first (G-11): error-recovery tests precede AST-shape tests in the commit history.

### L2 -- Semantic Analysis
Rules §16.1-2 and 6-7: applicability, resolution via the unchanged `veridikt_graph`, exhaustive match, intent-graph construction from `.veridikt` source -- `veridikt ask` works on `.veridikt` projects from this milestone. Exit: the Phase 1 query test suite passes verbatim against a `.veridikt` fixture project; `E0201/E0203/E0204/E0205` parity with Phase 1.

### L3 -- Type Checking + Bytecode
Rules §16.3-5 + §16's emit rule (tags `E0503`, sealed `E0504`, error unions `E0505/E0506`, state permissions `E0507/E0508`, `E0512`). Bytecode per §17.2-3 with the versioned header (D-035); `veridiktc build` emits `.veridiktc`; disassembler for tests. Intent-preserving opcodes are in from this milestone -- not retrofittable (G-8). Any opcode-set change here requires a D-entry. Exit: golden-disassembly tests per construct; a bad-version file is rejected with a clean error; `E0507` fires when `affects` is removed from the canonical example.
...
Write a real multi-module service in Veridikt end to end (the §19 example grown to 3+ modules, a workflow, and an external), run it, query it statically and live. Exit: the service handles a scripted load run; `veridikt ask path(<entrypoint>, Payment.ledger)` and `veridikt trace` agree with observed behavior; a written L7 report mirrors the T10 protocol. **Optimization is unlocked only after this milestone (G-9).**
...
2. **T10 fails on cost-of-intent (criterion 4).** If annotation takes too long, the fix is ergonomics inside Phase 1 (better `E` messages, `veridikt init` suggesting annotation stubs for high-churn modules), never weakening the gate.
