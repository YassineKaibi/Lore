# Veridikt Specification

**Version:** 0.2 (Implementation-Ready)
**Status:** Authoritative. Supersedes v0.1 entirely.
**Companions:** `veridikt-decisions.md` (binding rationale, D-NNN refs), `veridikt-guidelines.md` (process), `veridikt-roadmap.md` (milestones).
...
11. [Project Manifest: veridikt.toml](#11-project-manifest-veridikttoml)
12. [CLI](#12-cli)
13. [Shared Data Contracts (Rust)](#13-shared-data-contracts)
14. [Phase 2: The Veridikt Language](#14-phase-2-the-veridikt-language)
...
A developer has never been able to ask their system a question and trust the answer. Source code is the truth but illegible at scale; artifacts about code (comments, docs, wikis) are legible but untrustworthy. Veridikt ends that trade-off by maintaining **one intent graph with two layers**:
...
- **Declared layer** -- the *why*. Built from `@veridikt` annotation blocks (Phase 1) or `.veridikt` intent blocks (Phase 2). Human knowledge that no analysis can derive: `purpose`, `because`, `unknown`, `assumes`, `owner`.
...
**Phases.** Phase 1 is the language-agnostic tool: scan annotations, derive structure, reconcile, answer `veridikt ask`, lint in CI, export, serve MCP. Phase 2 is the dedicated `.veridikt` language in which grounding is total: the compiler checks every effect declaration against actual state-API call sites. Phase 2 MUST NOT begin before the T10 gate passes (D-039).
...
The intent model is shared verbatim between phases (`veridikt_intent` crate). One clause grammar, one AST, one graph. Phase 1 delivers clauses inside host-language comments; Phase 2 delivers them inside `.veridikt` syntax. Nothing else differs.
...
Removed from v0.1: `version`, `changed` (VCS-derived instead; see `veridikt history`, D-004).
...
- "R" is enforced by `veridikt lint` in Phase 1 (`E0201` missing required intent) and by the compiler in Phase 2.
- Requirement rows (R / Rec) apply only to nodes declared by an intent block; nodes that exist only via a `veridikt.toml` mapping or the derived layer are exempt (D-046).
...
|---|---|---|---|
| Module | Concept boundary; owns members | `kind: module` / veridikt.toml mapping | `def module` |
...
| External | Typed boundary to a non-Veridikt dependency (D-009) | `kind: external` | `def external` |
...
ref ::= Ident ("." Ident)*
```
Refs are unresolved at parse time (`veridikt_intent` stores segments + span); resolution happens in `veridikt_graph` (§6.3). All cross-module refs MUST be fully qualified -- there is no implicit context in either phase.
...
### 6.3 Resolution (runs in `veridikt_graph`, shared by both phases)
...
| `callers(F)` | reverse | `Triggers` ∪ `Calls` into F | transitive over same |
| `callees(F)` | forward | `Triggers` ∪ `Calls` out of F | transitive over same |
...
### 7.1 The `@veridikt` block

A block is a run of **contiguous** host-language line comments whose first content line is exactly `@veridikt`. The scanner strips each line's comment token plus at most one following space, preserves newlines, and reassembles the remainder into one intent-block text. Blank line or non-comment line ends the block. An `@veridikt` marker line whose immediately preceding line is itself a comment does **not** begin a block (the run started earlier); because that silently discards an intended block — most often a `///`/`//!` doc comment sitting directly above `// @veridikt` — the scanner emits `W0110` pointing at the marker (D-085).
...
```ebnf
veridikt_block    ::= marker_line ( binding_line | clause_line )*
marker_line   ::= "@veridikt"
...
| Rust | `//` | `function_item`, `struct_item`, `enum_item`, `trait_item`, `static_item`, `const_item`, `mod_item`, `field_declaration` | `attribute_item` |
...
1. `veridikt.toml [modules]` maps path globs → module names. Every file matching a glob belongs to that module. Glob `*` matches within a single path segment and MUST NOT cross `/`; `**` is the segment-crossing wildcard (D-082). Overlapping globs assigning two different modules to one file: `E0103`.
2. A top-of-file block with `kind: module` (or `service`/`workflow`) scopes that file and overrides (1) for that file.
3. A file in neither: annotated subjects get qname `_orphan.<name>` and `W0208`. To avoid this for root-level files, `veridikt init` proposes a `<root>/*` glob alongside the per-directory globs (D-081).
...
Each language contributes: (a) a tree-sitter grammar dependency, (b) declaration queries (`.scm`), (c) call-expression queries, (d) import-resolution rules, (e) a mutator list. From T8 these five artifacts are packaged as a **language pack** (§8.6, D-070): one `veridikt-lang.toml` plus two `.scm` query files plus a mandatory conformance fixture suite, loaded through one generic adapter -- there is no per-language Rust code (sole exception: named custom import strategies, D-071). The pre-T8 layout (`veridikt_derive/src/lang/<name>/`) is superseded; Python and TypeScript migrate onto packs at T8 with behavior unchanged (D-070h).
...
```
packs/go/
  veridikt-lang.toml        # pack manifest (§8.6.1)
...
`veridikt_cli` parses and validates packs and passes them to `veridikt_annotations` / `veridikt_derive` as data plus a grammar handle; the adapters compile the query files at activation (D-070d). A pack that fails any check in this section MUST NOT be activated -- the language is simply not loaded, never partially.

#### 8.6.1 Pack manifest: `veridikt-lang.toml`
...
|---|---|---|
| `[pack] name` | R | Language id; MUST equal the pack directory name and is the value used in `veridikt.toml [project] languages`. |
...
*Note: a tier is the honest statement of roadmap risk 1's "ship it scanner+binder-only and say so" -- declared in data, surfaced by `veridikt stats`.*
...
A pack missing a mandatory class (or with an empty one), or whose suite has not passed for its exact content, MUST NOT be activated (`E0415`). Builtin packs are enforced in CI by the **conformance harness** -- a `veridikt_cli` test running every embedded pack's suite through the real scan→bind→derive pipeline -- so a failing pack cannot ship. Future external packs (reserved, like WASM grammars) run the suite at first load.

A pack's `fixtures/` directory is conformance test data, not project source: the CLI source walk skips the `fixtures/` directory of any language pack (a `fixtures` dir beside a `veridikt-lang.toml`), so its deliberately-malformed inputs never enter `veridikt scan`/`lint` (D-075).

---

## 9. Reconciliation and Staleness

Runs in `veridikt_graph` after both layers are built. Pure function of (declared edges, derived edges, source text, git metadata).
...
`0` success / no error-severity findings; `1` error-severity findings; `2` usage or input parse error; `3` internal error (panic boundary). For `veridikt ask`: an unparseable query, an argument naming no node (message names the nearest existing qname), a wrong-kind filter argument, or `--all` without `--max-len` is exit 2; graph findings never fail `ask` (D-053).
...
Unknown keys: `E0401`. A known key with an invalid value: `E0403` (D-043). Missing manifest: every command except `veridikt init` fails with `E0402` and the suggestion to run `veridikt init`. `[lint]` keys MUST be W-codes from the §18 registry (`E0401` otherwise -- E findings can never be silenced) and values `"warn"` or `"off"` (`E0403` otherwise); `"off"` suppresses the code from lint output and the exit-code computation, including strict-promoted instances, while `ask`/`show` stay unfiltered (D-056).

---

## 12. CLI

Single binary `veridikt` (crate `veridikt_cli`).

| Command | Does | Milestone |
|---|---|---|
| `veridikt init` | Write a starter `veridikt.toml`; detect languages; propose `[modules]` globs from directory names, plus a disjoint `<root>/*` glob for root-level source files (D-081) | T1 |
| `veridikt scan [--json]` | Scanner+binder only: list every annotation block, its subject, qname, kind | T1 |
| `veridikt ask "<query>" [--json] [--all --max-len N]` | §10 | T4 |
| `veridikt lint [--json] [--no-stale]` | Resolution checks, required intent, applicability, depends_on surface, hygiene (`W0210`-`W0212`), reconciliation, staleness; exit per §10.5 | T3 (structural) → T7 (full) |
| `veridikt stats [--json]` | Coverage: nodes by kind/origin, % declared intent per kind, claims by status (T7), unresolved_calls + ambiguous_derived_names counts (D-065) | T6 (counts) → T7 (claim statuses) |
| `veridikt history <qname> [--json]` | §9.3 (D-059) | T5 |
| `veridikt graph --dot [--focus <qname> --depth N]` | Graphviz export (D-038) | T8 |
| `veridikt mcp` | MCP server (stdio): tools `veridikt_ask`, `veridikt_show`, `veridikt_lint`, `veridikt_history` mapping 1:1 to the JSON outputs (D-037, mechanics D-079, usage guidance D-080) | T9 |
| `veridiktc build / run` | Phase 2 compiler/VM driver | L1+ |
...
// Language pack as pure data (§8.6, D-070/D-071). veridikt_cli parses and
// validates a pack, then hands the PackSpec to veridikt_annotations / veridikt_derive
// alongside the tree-sitter grammar handle as a *separate* argument -- so
// veridikt_intent never depends on tree-sitter (D-070d). bind_scm / derive_scm
...
pub struct PackSpec {
    pub name: String,                    // == pack dir name, == veridikt.toml language id
...
pub enum ImportStrategy {                // §8.6.1, D-071 built-in strategy library
    ...
    Custom         { name: String },     // selects a registered veridikt_derive impl; needs a D-entry
}
...
pub struct Graph {
    ...
    pub findings: Vec<Finding>,          // veridikt_intent::Finding -- E/W codes with spans (§18, D-040)
    pub attributions: HashMap<QName, Vec<usize>>, // node -> indices into findings (D-049 attribution, public per D-055)
}

// CODEOWNERS data passed into veridikt_graph::build by the CLI (D-058) --
...
// The derived layer as data, passed into veridikt_graph::build by the CLI from
// veridikt_derive's output (the graph never depends on the derive crate, §13).
...
Crate layout: `crates/{veridikt_intent, veridikt_annotations, veridikt_derive, veridikt_graph, veridikt_cli, veridikt_lexer, veridikt_parser, veridikt_semantic, veridikt_bytecode, veridikt_vm}`.

---

## 14. Phase 2: The Veridikt Language

Everything in §§3--6 applies unchanged; this section adds what only the language has. File extension `.veridikt`; compiler `veridiktc`.
...
Checked by `veridikt_semantic`, which also produces `IntentNode`s and feeds `veridikt_graph` (identical graph code as Phase 1).
...
7. **Effect derivation:** the compiler derives `Calls`, `Affects`, `Reads` edges from bodies and `Emits`/`Handles` from `emit`/handler registration -- so in Phase 2 every claim kind is reconciled and `Unverifiable` disappears for `.veridikt` code.
...
Stack-based VM; call frames hold locals + return address. Errors are values: `raise` constructs an `Err` and returns it -- no unwinding, no exceptions. Services are VM units: `veridiktc run <Service>` registers routes and serves. Scaling/orchestration permanently out of scope.

### 17.2 Bytecode

`.veridiktc` file: magic `VERIDIKTC`, `u16 version = 1`, `u16 reserved`, then constant pool, function table, route table, state table, event table, code. Unknown version → clean rejection before execution (D-035). Intent payloads (`unknown` messages, `because` strings, state/event ids) are in the constant pool so `veridikt trace` can read them.
...
### 17.4 Runtime bindings -- `veridikt.runtime.toml` (D-031, D-033)
...
| Band | Area |
|---|---|
| E010x/W010x | Scanner & binder (`E0102` unbound annotation, `E0103` overlapping module globs, `E0104` ambiguous assignment target, `E0105` step outside workflow, `E0106` invalid kind value, `E0107` invalid name value, `E0108` scoping block missing name, `E0109` non-scoping block missing name in a scan-tier language -- D-070; `W0110` `@veridikt` marker preceded by a contiguous comment line, so the block is not recognized -- D-085) |
...
Severity defaults are the letter (E/W); `veridikt.toml [lint]` may override W↔off (domains and application point per D-056) and promote per `[policy]` (`unknown` promotes `W0213`, D-057; `stale` promotes `W0301` at T7); module `enforcement: strict` promotes that module's W findings from bands 02x/03x to errors (attribution and nearest-module rule per D-049).

---

## 19. Canonical Example

### Phase 1 -- annotated Python (`src/payments/service.py`, module `Payment` via veridikt.toml)
...
`veridikt lint` here: `affects: Payment.ledger` → **Verified**; `reads: Payment.balances` → Verified; `emits` → Unverifiable (P1); plus `W0302` the day someone deletes the `ledger.append` line, and `W0301` the day the body changes without the block changing.

### Phase 2 -- `payment.veridikt` (excerpt showing every resolved construct)
...
*End of specification. Deviations require a `veridikt-decisions.md` entry (guideline G-1).*
